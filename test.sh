...
try 2 "main(){bar(); baz();} bar(){1;} baz(){2;}"
try 6 "main(){c=bar(); c+1;} bar(){a=2;b=3; a+b;}"
try 6 "main(){a=bar(); a+1;} bar(){a=2;b=3; a+b;}"
try 6 "main(){bar()+baz();} bar(){2;} baz(){4;}"
try 1 "main(){bar(1);} bar(a){a;}"
try 2 "main(){bar(1);} bar(a){a+1;}"
try 3 "main(){bar(baz());} bar(a){a;} baz(){3;}"
try 3 "main(){bar(1, 2);} bar(a, b){a+b;}"
try 1 "main(){bar();} bar(){return 1; return 2;}"
try 2 "main(){bar(1);} bar(a){if(a==1)return 2; return 3;}"
try 55 "main(){f(10);} f(n){if(n==0)return 0; if(n==1)return 1; return f(n-1)+f(n-2);}"
echo OK
